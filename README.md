...
- `claude/rules/` — standing conventions installed into `~/.claude/rules/`: `jira.md` (status-workflow lookup, ticket key on every PR, epic linkage) and `plandb.md` (when/how to use PlanDB for task decomposition and parallelism). Add more `*.md` files here for any other cross-project convention.
